...
- **语言指令**：始终使用**简体中文**进行思考、回复和知识库的编写。
- **角色定义**：你正在维护一个 **LLM Wiki**（根据 [Karpathy 的规范](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f))，任务是将碎片化的信息编译成结构化、高度相互链接的 Obsidian 知识库。

### 核心目录与权限边界 (Immutability & Architecture)
你必须严格遵守以下文件操作权限和不可逾越的底线：
- `/raw/` (原始资料只读层 - Immutable)：
	  - **绝对只读**：存放原始素材、文案、资料，它是事实的唯一真相来源。
	  - **禁止修改此目录下的任何文件**。
	  - **禁止删除此目录下的任何文件**。
- `/assets/` (媒体资产层)：
