...
若任何步骤执行了 `Codex mcp add`，**告知用户重启 Codex**，修复后重新执行 `/init` 验证。
...
| ⚙️ **已编译未注册** | 编译产物存在，但工具不响应 | 执行 `Codex mcp add`（见各节） |
...
```bash
Codex mcp remove pingcode 2>/dev/null
Codex mcp add pingcode node -s user \
...
```bash
Codex mcp remove kibana 2>/dev/null
Codex mcp add kibana mcp-server-kibana -s user \
...
```bash
Codex mcp remove yearning 2>/dev/null
Codex mcp add yearning node -s user \
  -e YEARNING_URL=<值> \
...
<若有任何 mcp add 操作>
⚠️ 已更新 MCP 注册，请**重启 Codex**，然后重新执行 `/init` 确认生效。
