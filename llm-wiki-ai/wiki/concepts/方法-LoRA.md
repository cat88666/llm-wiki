...
## 在知识库中的出现
- [[主题-LLM生产化与评估]] summary：LoRA 微调与 RAG 是 LLM 能力增强的两种互补路径，LoRA 适合需要风格/格式定制的场景
- [[方法-正则化]] concept：LoRA 的低秩约束是隐式正则化，与 L2 正则的作用类似（限制参数更新自由度）
...
- [[工具-vLLM]]：LoRA 微调后的模型可以用 vLLM 部署服务，vLLM 支持 LoRA 适配器的动态加载
