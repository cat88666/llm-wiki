...
- 这也是后续 [[FlashAttention]]、稀疏注意力、窗口注意力和 [[KV-Cache]] 出现的根本原因。

## 与其他概念的关系
- [[多头注意力]]：把同一个注意力问题拆到多个子空间并行求解。
- [[KV-Cache]]：把历史 token 的 K/V 缓存下来，避免自回归推理时重复计算。
- [[FlashAttention]]：不改变注意力结果，只重排计算顺序以减少显存读写。
- [[Tokenizer]]：决定输入被切成什么粒度，进而影响注意力看到的基本单位。
