...
## 第一性原理
- 标准 [[Self-Attention]] 的数学并不复杂，真正慢的是中间巨大矩阵在 HBM 和片上缓存之间反复搬运。
...
## 与其他概念的关系
- [[Self-Attention]]：FlashAttention 是其高效实现，而不是替代机制。
- [[多头注意力]]：对每个头分别应用同样的分块思想。
- [[KV-Cache]]：两者都服务推理优化，但一个减少重复计算，一个减少单次 IO。
- [[分布式训练]]：可与序列并行、张量并行结合，处理更长序列训练。
