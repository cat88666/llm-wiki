...
- [[多头注意力]] 中 MHA、GQA、MQA 的差异，本质上也是在改 KV 缓存成本。

## 与其他概念的关系
- [[Self-Attention]]：缓存的是注意力层中的历史 K/V，而不是最终 hidden state。
- [[多头注意力]]：K/V 是否共享直接决定缓存体积。
- [[FlashAttention]]：FlashAttention 优化单次 attention 的 IO，KV Cache 避免历史重算，两者正交。
- [[分布式训练]]：推理并行时缓存也需要按张量切分策略同步布局。
