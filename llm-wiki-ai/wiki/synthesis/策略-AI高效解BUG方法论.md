...
> 来源类型：内部实操记录，不对应 `raw/` 下单一原始文件。

## 快速上手（3 步）

> 新同学看这里，3 步完成第一次 AI 排障。
...
AI 自动检查 PingCode / Kibana / Yearning 三个 MCP 的状态，缺什么补什么，输出"✅ 环境就绪"后继续。

**第 2 步：粘贴 bug 描述，触发全链路排障**

```text
/bug <PingCode 工单号，或直接粘贴 bug 描述>
```

AI 会自动：读单→分析→搜代码→按需查库/查日志→给出修复代码。

**第 3 步：Review AI 修改，自行 commit 合并**

AI 不执行 `git commit`/`push`，改完代码就停下等你审核。
...
> 配置完成后在 Codex 中验证：输入"帮我查一下当前有哪些数据库"，返回列表即成功。  
> **详细安装流程（含自动检测和重建步骤）**：见 `.claude/skills/bug/SKILL.md` 步骤 -1。
...
> **详细安装流程（含 Cookie 解密步骤）**：见 `.claude/skills/bug/SKILL.md` 步骤 -1。
