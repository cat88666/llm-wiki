...
- **Attention 复杂度是 O(n²)**：序列翻倍，计算量四倍增长，这是长文本处理的根本瓶颈，需要稀疏/线性/低秩近似或 [[FlashAttention]] 优化
...
## 关键概念
- [[Self-Attention]]：Transformer 的核心计算单元，通过 Q/K/V 实现动态全局依赖建模
- [[多头注意力]]：Self-Attention 的扩展，多个子空间并行学习不同类型的依赖关系
- [[KV-Cache]]：推理阶段缓存 K/V 矩阵，避免重复计算历史 token 的注意力，是推理加速的基础
- [[FlashAttention]]：IO 感知的注意力实现，通过减少 HBM 读写实现 2-4x 加速，不改变数学结果
...
## 延伸方向
- 可拆分为 [[Self-Attention]] 和 [[多头注意力]] 的详细 concept 页
- 可延伸到 [[KV-Cache]] 和 [[FlashAttention]] 的工程优化 concept 页
- 可连接到 [[LLM核心机制与推理优化]] summary 页（GQA、分布式训练）
