...
## 知识核心
- **KV Cache 是推理的核心瓶颈**：随序列长度线性增长，对于 70B 模型，单序列 KV Cache 可达数 GB；[[工具-vLLM]] 的 PagedAttention 借鉴操作系统虚拟内存思路，将显存利用率从约 60% 提升到 90%+
...
## 关键概念
- [[机制-KV-Cache]]：推理时缓存历史 token 的 K/V 矩阵，是自回归生成可用的核心加速手段
- [[机制-FlashAttention]]：IO 感知的注意力实现，通过减少 HBM 访问实现速度和显存双重优化
- [[方法-分布式训练]]：DP/TP/PP/ZeRO 四种策略的组合，解决大模型无法在单卡上训练的问题
- [[概念-Tokenizer]]：BPE/WordPiece/SentencePiece，决定文本到 token 的映射效率
...
## 延伸方向
- 可拆分为 [[工具-vLLM]] 实体页（PagedAttention 详细机制、部署配置）
- 可延伸到 [[方法-分布式训练]] concept 页（ZeRO-1/2/3 的差异）
- 可连接到 [[主题-LLM生产化与评估]] summary 页（推理延迟优化与成本控制）
