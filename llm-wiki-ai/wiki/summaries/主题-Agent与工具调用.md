...
## 关键概念
- [[概念-Function-Calling]]：LLM 结构化工具调用的基础机制，工具描述的设计直接决定调用准确率
- [[方法-Prompt工程]]：Agent 的系统提示设计是 Prompt 工程的高级应用，需要描述角色、工具列表、推理格式
- [[概念-RAG]]：Agent 系统中 RAG 通常作为一个工具（知识检索工具）被调用，而不是独立系统
...
- 多 Agent 的调试困难：当 Sub-Agent 的输出作为 Orchestrator 的输入时，错误传播链很长，需要每个 Agent 的 trace 可以独立查看，[[工具-LangFuse]] 等观测工具是必须的

## 延伸方向
- 可拆分为 [[概念-Function-Calling]] concept 页（工具描述规范、结构化输出）
- 可连接到 [[主题-LLM生产化与评估]] summary（Agent 的 trace_id 追踪、LangFuse 集成）
