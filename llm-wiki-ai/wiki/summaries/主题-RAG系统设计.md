...
- 向量数据库选型要考虑：过滤能力（metadata）、更新频率、部署方式（本地/云端）；[[工具-Qdrant]] 是开源方案中工程成熟度较高的选择
...
## 关键概念
- [[概念-RAG]]：检索增强生成的整体架构，解决 LLM 知识截止和幻觉问题
- [[方法-混合检索]]：向量检索 + BM25 + RRF 融合，是 RAG 检索层的最佳实践
- [[工具-Reranker]]：Cross-Encoder 精排，是 RAG 精度的最大杠杆
- [[方法-Prompt工程]]：RAG 的上下文组装是 Prompt 工程的核心应用场景
...
## 延伸方向
- 可拆分为 [[概念-RAG]] concept 页（端到端流程详解）
- 可拆分为 [[方法-混合检索]] concept 页（RRF 算法、BM25 参数调优）
- 可拆分为 [[工具-Reranker]] 实体页（模型选型、部署方式）
- 可连接到 [[主题-LLM生产化与评估]] summary（Faithfulness 指标评估）
